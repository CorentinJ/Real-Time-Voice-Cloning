...
def add_train_stats(model, hparams):
    with tf.compat.v1.variable_scope("stats") as scope:
        ...
        if hparams.tacotron_teacher_forcing_mode == "scheduled":
            ...
            tf.compat.v1.summary.scalar("teacher_forcing_ratio", model.ratio)
        ...
        tf.compat.v1.summary.scalar("max_gradient_norm", tf.reduce_max(
            input_tensor=gradient_norms))  # visualize
...
def add_eval_stats(summary_writer, step, linear_loss, before_loss, after_loss, stop_token_loss,
    ...
    values = [
        tf.compat.v1.Summary.Value(
            tag="Tacotron_eval_model/eval_stats/eval_before_loss", simple_value=before_loss),
        tf.compat.v1.Summary.Value(
            tag="Tacotron_eval_model/eval_stats/eval_after_loss", simple_value=after_loss),
        tf.compat.v1.Summary.Value(
            tag="Tacotron_eval_model/eval_stats/stop_token_loss", simple_value=stop_token_loss),
        tf.compat.v1.Summary.Value(
            tag="Tacotron_eval_model/eval_stats/eval_loss", simple_value=loss),
    ]
    if linear_loss is not None:
        values.append(tf.compat.v1.Summary.Value(
            tag="Tacotron_eval_model/eval_stats/eval_linear_loss", simple_value=linear_loss))
        ...
        try:
            ...
            if args.restore:
                ...
                try:
                    ...
                    if checkpoint_state and checkpoint_state.model_checkpoint_path:
                        ...
                        saver.save(sess, checkpoint_fpath,
            ...
            while not coord.should_stop() and step < args.tacotron_train_steps:
                ...
                if step % args.eval_interval == 0:
                    ...
                    stop_token_loss = sum(
                        stop_token_losses) / len(stop_token_losses)
                ...
                if step % args.checkpoint_interval == 0 or step == args.tacotron_train_steps:
                    ...
                    audio.save_wav(wav,
                                   os.path.join(
                                       wav_dir, "step-{}-wave-from-mel.wav".format(step)),
                                   sr=hparams.sample_rate)

                    # save alignment plot to disk (control purposes)
                    plot.plot_alignment(alignment,
                                        os.path.join(
                                            plot_dir, "step-{}-align.png".format(step)),
                                        title="{}, {}, step={}, loss={:.5f}".format("Tacotron",
                                                                                    time_string(),
                                                                                    step, loss),
