...
class LSA(nn.Module):
    def __init__(self, attn_dim, kernel_size=31, filters=32):
        super().__init__()
        self.conv = nn.Conv1d(2, filters, padding=(
            kernel_size - 1) // 2, kernel_size=kernel_size, bias=False)
    ...
    def forward(self, encoder_seq_proj, query, t):
        ...
        location = torch.cat([self.cumulative.unsqueeze(
            1), self.attention.unsqueeze(1)], dim=1)
...
class Decoder(nn.Module):
    ...
    def __init__(self, n_mels, decoder_dims, lstm_dims, dropout, speaker_embedding_size):
        ...
        self.attn_rnn = nn.GRUCell(
            decoder_dims + prenet_dims[1] + speaker_embedding_size, decoder_dims)
        self.rnn_input = nn.Linear(
            2 * decoder_dims + speaker_embedding_size, lstm_dims)
...
class Tacotron(nn.Module):
    def __init__(self, embed_dims, num_chars, encoder_dims, decoder_dims, n_mels,
        ...
        self.encoder_proj = nn.Linear(
            decoder_dims + speaker_embedding_size, decoder_dims, bias=False)
        self.decoder = Decoder(n_mels, decoder_dims,
                               lstm_dims, dropout, speaker_embedding_size)
    ...
    def forward(self, x, m, speaker_embedding):
        ...
        context_vec = torch.zeros(
            batch_size, self.decoder_dims + self.speaker_embedding_size, device=device)
