...
if __name__ == "__main__":
    ...
    parser.add_argument("run_id", type=str, help="Name for this model. By default, training outputs will be stored to "
                                                 "saved_models/<run_id>/. If a model state "
                                                 "from the same run ID was previously saved, the training will "
                                                 "restart from there. Pass -f to overwrite saved "
                                                 "states and restart from scratch.")
    parser.add_argument("datasets_root", type=Path, help="Path to the directory containing your SV2TTS directory. "
                                                         "Specifying --syn_dir or --voc_dir "
                                                         "will take priority over this argument.")
    parser.add_argument("--syn_dir", type=Path, default=argparse.SUPPRESS, help="Path to the synthesizer "
                                                                                "directory that contains the "
                                                                                "ground truth mel "
                                                                                "spectrograms, "
                                                                                "the wavs and the embeds. Defaults to "
