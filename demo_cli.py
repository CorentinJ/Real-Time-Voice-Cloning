...
Test your Real-Time-Voice-Cloning configuration with the
Demo CLI defined in this module. This module intents to
give a terminal-based short introduction/test of the code's
...
if __name__ == "__main__":
    ...
    if args.cpu:
        # Hide GPUs from Pytorch to enforce CPU processing
    ...
    if not args.no_mp3_support:
        try:
            ...
            print(
                "Librosa will is unable to open .mp3 files with no additional software being installed.\n"
    ...
    """
    Forward an audio waveform of zeros that lasts 1 second. Notice, how we can get the encoder's
    ...
    now, we will simply hide it like this:
    """
    ...
    """
    For the sake of making this test short, we will pass a short target length. The target length 
    is the length of the .wav segments that are processed parallelly. E.g., for audio sampled 
    ...
    while True:
        try:
            ...
            if in_fpath.suffix.lower() == ".mp3" and args.no_mp3_support:
                print("Can't use .mp3 files. Please retry.")
                continue
            ...
            1. Directly load from the filepath:
            """
            preprocessed_wav = encoder.preprocess_wav(in_fpath)

            """
            2. If the wav is already loaded:
            """
            ...
            # If seed is specified, reset torch seed, and enforce synthesizer reload
            if args.seed is not None:
                torch.manual_seed(args.seed)
                synthesizer = Synthesizer(args.syn_model_fpath)

            # The synthesizer works in batch mode, so you need to put your data in a list or numpy array
            texts = [text]
            embeds = [embed]
